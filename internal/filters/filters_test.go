...
func TestPredicateFilter(t *testing.T) {
	...
	event3 := &core.LogEvent{
		Properties: map[string]any{"Message": "exclude me"},
	}
	...
	event4 := &core.LogEvent{
		Properties: map[string]any{"Message": "include me"},
	}
...
func TestExpressionFilter(t *testing.T) {
	...
	event3 := &core.LogEvent{
		Properties: map[string]any{"Path": "/api/users"},
	}
	...
	event4 := &core.LogEvent{
		Properties: map[string]any{"Path": "/health"},
	}
	...
	event5 := &core.LogEvent{
		Properties: map[string]any{"Message": "An error occurred"},
	}
...
func TestHashSamplingFilter(t *testing.T) {
	...
	userId := "user123"
	event1 := &core.LogEvent{
		Properties: map[string]any{"UserId": userId},
	}
	event2 := &core.LogEvent{
		Properties: map[string]any{"UserId": userId},
	}
	...
	for i := 0; i < total; i++ {
		event := &core.LogEvent{
			Properties: map[string]any{"UserId": fmt.Sprintf("user%d", i)},
		}
		if filter.IsEnabled(event) {
			passed++
		}
...
func TestCompositeFilters(t *testing.T) {
	...
	event5 := &core.LogEvent{
		Properties: map[string]any{"Exclude": true},
	}
