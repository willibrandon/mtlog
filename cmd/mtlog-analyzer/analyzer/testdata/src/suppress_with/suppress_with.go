...
func testSuppressWithOddArgumentsDiagnostic() {
	log := &Logger{}
	
	// MTLOG009 is suppressed via analyzer flags, so these should NOT produce diagnostics
	log.With("key1", "value1", "key2") // No diagnostic expected (MTLOG009 suppressed)
	log.With("k1", "v1", "k2", "v2", "k3") // No diagnostic expected (MTLOG009 suppressed)
