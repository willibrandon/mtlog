...
func testContextKeyConstants() {
	...
	log = log.ForContext("user_id", 123)     // want `\[MTLOG007\] suggestion: consider defining a constant for commonly used context key 'user_id'`
	log = log.ForContext("user_id", 456)     // want `\[MTLOG007\] suggestion: consider defining a constant for commonly used context key 'user_id'`
	...
	log = log.ForContext("request_id", "abc-123")  // want `\[MTLOG007\] suggestion: consider defining a constant for commonly used context key 'request_id'`
	log.ForContext("request_id", "def-456").Information("Request") // want `\[MTLOG007\] suggestion: consider defining a constant for commonly used context key 'request_id'`
