...
func PushProperty(ctx context.Context, name string, value interface{}) context.Context {
	// Handle nil context
	if ctx == nil {
		ctx = context.Background()
	}
	...
	if existing := ctx.Value(logContextKey{}); existing != nil {
		if lcv, ok := existing.(*logContextValue); ok {
			...
			properties = make(map[string]interface{}, len(lcv.properties)+1)
			for k, v := range lcv.properties {
				properties[k] = v
			}
		}
	}
...
func getLogContextProperties(ctx context.Context) map[string]interface{} {
	if ctx == nil {
		return nil
	}
	...
	if value := ctx.Value(logContextKey{}); value != nil {
		if lcv, ok := value.(*logContextValue); ok {
			...
			props := make(map[string]interface{}, len(lcv.properties))
			for k, v := range lcv.properties {
				props[k] = v
			}
			return props
