	...
	testEvents := []*core.LogEvent{
		{
			...
			Properties:      map[string]interface{}{"Component": "DurableTest"},
		},
		{
			Timestamp:       time.Now(),
			Level:          core.InformationLevel,
...
func TestDurableSink_BufferFileCreation(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		event := &core.LogEvent{
			...
			Properties:      map[string]interface{}{"Test": "BufferRotation", "Index": i},
		}
		durableSink.Emit(event)
	}
...
func TestDurableSink_CleanupProcess(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		event := &core.LogEvent{
			...
			Properties:      map[string]interface{}{"Test": "Cleanup", "Index": i},
		}
		durableSink.Emit(event)
	}
...
func TestDurableSink_MultipleInstances(t *testing.T) {
	...
	for i, durableSink := range durableSinks {
		event := &core.LogEvent{
			...
			Properties:      map[string]interface{}{"Instance": i},
		}
		durableSink.Emit(event)
	}
